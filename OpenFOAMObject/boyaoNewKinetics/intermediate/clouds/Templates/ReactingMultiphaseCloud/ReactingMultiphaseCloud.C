...
{
  Info << "call writeFields in reactingMultiphaseCloud " << nl;
    if (this->compositionModel_.valid())
    {
        Info << "before call writeFields in ReactingMultiphaseParcel " << nl;
        CloudType::particleType::writeFields(*this, this->composition());
          Info << "after call writeFields in ReactingMultiphaseParcel " << nl;
    }
}
