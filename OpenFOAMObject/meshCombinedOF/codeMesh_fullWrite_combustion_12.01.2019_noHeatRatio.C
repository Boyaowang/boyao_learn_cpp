...
{
    double COCO2Ratio = 4.3*Foam::exp(-3390./temperature);
...
void discretized_psm(void)
{
    ...
    scalar Y_H2, Y_CO, Y_CH4, Y_H2O, Y_G, Y_CO2;
    scalar char_yield[FINE_PARTICLE_MESH];
    scalar beta_r[4];
    scalar D_g, D_gAB;
    scalar hmi, hmia, beta_d, /*eta, conc,*/ alpha, conc_O2, conc_O2_correct, O2_diff_suface ,omega_cS;
    scalar high, low;
...
//             R_product[H2_index] = R_product[H2_index] + Y_H2/Y_G*(R_devol_reac[1-1][i-1] + R_devol_reac[2-1][i-1]); /* (kg/s) */
        ...
        *Foam::pow((rho_g*2.0*rb*vel_mag/mu),0.6));
        ...
        for (i = 1; i <= FINE_PARTICLE_MESH; i++)
        {
            Info << "Stop here !!!!!!!" << total_devol[i-1] << nl;
