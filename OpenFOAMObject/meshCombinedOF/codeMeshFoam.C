...
static label VERBOSITY, SAVE_EVERY, FINE_PARTICLE_MESH;
...
void readCoeff(const Time & time) {
  ...
  particleInfoDict_.lookup("VERBOSITY")>> VERBOSITY;
  particleInfoDict_.lookup("SAVE_EVERY")>> SAVE_EVERY;
  particleInfoDict_.lookup("TIME_DT")>> TIME_DT;
  particleInfoDict_.lookup("TIME_TOTAL")>> TIME_TOTAL;
  particleInfoDict_.lookup("MOISTURE_FRACTION_DB")>> MOISTURE_FRACTION_DB;
  particleInfoDict_.lookup("PARTICLE_RADIUS")>> PARTICLE_RADIUS;
  particleInfoDict_.lookup("DRYING_SHRINKAGE_VOLUME")>> DRYING_SHRINKAGE_VOLUME;
  particleInfoDict_.lookup("T_REACTOR")>> T_REACTOR;
  particleInfoDict_.lookup("T_RADIATION")>> T_RADIATION;
  particleInfoDict_.lookup("MASS_FRAC_O2")>> MASS_FRAC_O2;
  particleInfoDict_.lookup("DENSITY_INITIAL")>> DENSITY_INITIAL;
  particleInfoDict_.lookup("FINE_PARTICLE_MESH")>> FINE_PARTICLE_MESH;
  particleInfoDict_.lookup("FINE_PARTICLE_MESH_DOUBLE")
                    >> FINE_PARTICLE_MESH_DOUBLE;
  particleInfoDict_.lookup("K_P_WET")>> K_P_WET;
  particleInfoDict_.lookup("K_P_DRY")>> K_P_DRY;
  particleInfoDict_.lookup("K_P_CHAR")>> K_P_CHAR;
...
scalar c_p(label layer, scalar T)
{
    ...
    scalar Y_mois = 1.0 - 1.0/(1.0 + MOISTURE_FRACTION_DB);
    //     scalar hcgas;
...
void discretized_psm(void)
{
    ...
    MAT *A, *LU;
    PERM *pivot;
    scalar xi[4][FINE_PARTICLE_MESH];
    scalar V_cell[FINE_PARTICLE_MESH],V_char_last[FINE_PARTICLE_MESH],V_char[FINE_PARTICLE_MESH];
    scalar xc[FINE_PARTICLE_MESH];
    scalar aw, ae, ap, ap0,Su_b,Sp_b;
    scalar Sp[FINE_PARTICLE_MESH];
    scalar Su[FINE_PARTICLE_MESH];
    scalar dt;
    ...
    //     scalar xi_t[4];
    scalar omega_c[FINE_PARTICLE_MESH];
    scalar Q_reaction[3][FINE_PARTICLE_MESH];
    scalar R_b[4][FINE_PARTICLE_MESH];
    scalar R_char_reac[4][FINE_PARTICLE_MESH];
    //     scalar R_char_reac1[FINE_PARTICLE_MESH], R_char_reac2[FINE_PARTICLE_MESH], R_char_reac3[FINE_PARTICLE_MESH], R_char_reac4[FINE_PARTICLE_MESH];
    scalar R_char_reac1S;//, R_char_reac2S, R_char_reac3S, R_char_reac4S;
    scalar m[4][FINE_PARTICLE_MESH],m_last[4][FINE_PARTICLE_MESH];
    //     scalar m_t[4];
    scalar total_mass;
    scalar S_star[FINE_PARTICLE_MESH], dSdT_star[FINE_PARTICLE_MESH];
    scalar char_from_devol[FINE_PARTICLE_MESH];
    scalar total_devol[FINE_PARTICLE_MESH];
    scalar R_product[7];
    scalar Q_equilibrate[3][FINE_PARTICLE_MESH];
    //     scalar delta_V[FINE_PARTICLE_MESH];
    scalar R_devol_reac[3][FINE_PARTICLE_MESH];
    // scalar conc_CO2, conc_H2O, conc_H2;
    // scalar Y_H2=0.0, Y_CO=0.0,Y_CH4=0.0,Y_G=0.0, Y_CO2=0.0;//Y_H2O=0.0,
    scalar char_yield[FINE_PARTICLE_MESH];
    scalar beta_r[4];
    scalar D_g, D_gAB;
    scalar hmi=0.0, hmia, beta_d, /*eta, conc,*/ alpha=0.0, conc_O2, conc_O2_correct, O2_diff_suface=0.0 ,omega_cS;
    scalar high=0.0, low=0.0;
    scalar dmdt[4][FINE_PARTICLE_MESH];
    scalar volume_sum;
    label /*allow_combustion[FINE_PARTICLE_MESH],*/ combustion_flag,combustion_layer;
    //     scalar heatFraction = 1.0; //the ratio between the heat absorbed by char to the total heat relased by char conversion
//     scalar R_H2_Sum[FINE_PARTICLE_MESH];
    scalar R_O2_mole_need = 0.;
    scalar R_O2_mole_need_list[FINE_PARTICLE_MESH];
    ...
    scalar ThermalTimeScale[FINE_PARTICLE_MESH];
    ...
    /* Set some parameters */
    dt = TIME_DT;
    output_every = SAVE_EVERY;//((label) (1.0/dt));
    t_stop = TIME_TOTAL;
...
//             R_product[H2_index] = R_product[H2_index] + Y_H2/Y_G*(R_devol_reac[1-1][i-1] + R_devol_reac[2-1][i-1]); /* (kg/s) */
        ...
        *Foam::pow((rho_g*2.0*rb*vel_mag/mu),0.6));
        ...
        e = - (view_factor*emissivity*SIGMA*Foam::pow(T_RADIATION,4.0) + h*T_inf + (xi[layer_1][0]*k_p(layer_1, T->ve[0]) + xi[layer_2][0]*k_p(layer_2, T->ve[0]) + xi[layer_3][0]*k_p(layer_3, T->ve[0]) + xi[layer_4][0]*k_p(layer_4, T->ve[0]))*T->ve[0]/(rb - xc[0]));
        q = d/a;
        delta_0 = 12.0*a*e;
