...
)
{
    ...
    scalar massGas =
        this->updateMassFraction(mass0*YMix[GAS], dMassGas, YGas_); // in ReactingParcel.C 149
    scalar massLiquid =
        this->updateMassFraction(mass0*YMix[LIQ], dMassLiquid, YLiquid_);
    scalar massSolid =
        this->updateMassFraction(mass0*YMix[SLD], dMassSolid, YSolid_);

    scalar massNew = max(massGas + massLiquid + massSolid, rootVSmall); //rootVSmall=1.0e-18;

    YMix[GAS] = massGas/massNew;
    YMix[LIQ] = massLiquid/massNew;
    YMix[SLD] = 1.0 - YMix[GAS] - YMix[LIQ];//That's smart!!
