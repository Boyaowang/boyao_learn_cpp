...
static float TIME_DT, TIME_TOTAL, PARTICLE_RADIUS, T_REACTOR, T_RADIATION, MASS_FRAC_O2, DENSITY_INITIAL, MOISTURE_FRACTION_DB, MOISTURE_FRACTION_WB, FINE_PARTICLE_MESH_DOUBLE, DRYING_SHRINKAGE_VOLUME, ASH_MASS_FRACTION_DB;
clock_t start;
...
void discretized_psm(void)
{
    ...
    for (i = 1; i <= FINE_PARTICLE_MESH; i++)
    {
        ...
        // printf("FINE_PARTICLE_MESHs: %10f \n",T->ve[i-1]);
    }
    ...
    while (t < t_stop)
    {
        ...
        for (i = FINE_PARTICLE_MESH; i >= 1; i--)
        {
            ...
            if (combustion_flag==0)
            {
                ...
                if (i == 1 && xi[layer_3][i-1] > MY_EXTREMELY_SMALL /*&& xi[layer_2][i-1]<=0.*/ )
                {
                    ...
                    Q_reaction[char_combustion][i-1] = R_char_reac[1-1][i-1]*(2.0*(omega_c[i-1]-1.0)*LHV[index_CO]+(2.0-omega_c[i-1])*LHV[index_CO2]-omega_c[i-1]*LHV[index_C]);/* + R_char_reac[2-1][i-1]*(2.0*LHV[index_CO]-LHV[index_C]-LHV[index_CO2]) + R_char_reac[3-1][i-1]*(LHV[index_CO]+LHV[index_H2]-LHV[index_C]-LHV[index_H2O]) + R_char_reac[4-1][i-1]*(LHV[index_CH4]-LHV[index_C]-2.0*LHV[index_H2])*/; /* kg/s*J/kg = W */
                ...
                else if (i != 1 && xi[layer_3][i-1] > MY_EXTREMELY_SMALL/* && xi[layer_2][i-1]<=0.*/ && xi[layer_3][i-2] <= MY_EXTREMELY_SMALL && xi[layer_2][i-2] <= MY_EXTREMELY_SMALL && xi[layer_1][i-2] <= MY_EXTREMELY_SMALL)
                {
                    ...
                    //             R_char_reac[4-1][i-1] = /*xi[layer_3][i-1]**/M_C*conc_H2*(beta_r[4-1]*beta_d/(beta_r[4-1]+beta_d))*surface_area(xc[i-1]);                            /* kg/mol*mol/m3*m/s*m2 = kg/s */
                    R_b[char_front][i-1] = R_char_reac[1-1][i-1] /*+ R_char_reac[2-1][i-1] + R_char_reac[3-1][i-1] + R_char_reac[4-1][i-1]*/;      /* rate of consumption of char (kg/s) */
                    Q_reaction[char_combustion][i-1] = R_char_reac[1-1][i-1]*(2.0*(omega_c[i-1]-1.0)*LHV[index_CO]+(2.0-omega_c[i-1])*LHV[index_CO2]-omega_c[i-1]*LHV[index_C]);
        ...
        for (i = FINE_PARTICLE_MESH; i >= 1; i--)
        {
            ...
            else if (i == 1)
            {
                ...
                if (i == combustion_layer+1)// combustion at i-1, NO combustion at i
                {
                    ...
                    high =  high - (R_char_reac1S*(2.0*(omega_cS-1.0)*LHV[index_CO]+(2.0-omega_cS)*LHV[index_CO2]-omega_cS*LHV[index_C]));
                    ...
                    low =  low - (R_char_reac1S*(2.0*(omega_cS-1.0)*LHV[index_CO]+(2.0-omega_cS)*LHV[index_CO2]-omega_cS*LHV[index_C])/* + R_char_reac2S*(2.0*LHV[index_CO]-LHV[index_C]-LHV[index_CO2]) + R_char_reac3S*(LHV[index_CO]+LHV[index_H2]-LHV[index_C]-LHV[index_H2O]) + R_char_reac4S*(LHV[index_CH4]-LHV[index_C]-2.0*LHV[index_H2])*/);
                ...
                else if (i == combustion_layer)// no combustion at i-1, combustion at i
                {
                    ...
                    high =  high - (R_char_reac1S*(2.0*(omega_cS-1.0)*LHV[index_CO]+(2.0-omega_cS)*LHV[index_CO2]-omega_cS*LHV[index_C])/* + R_char_reac2S*(2.0*LHV[index_CO]-LHV[index_C]-LHV[index_CO2]) + R_char_reac3S*(LHV[index_CO]+LHV[index_H2]-LHV[index_C]-LHV[index_H2O]) + R_char_reac4S*(LHV[index_CH4]-LHV[index_C]-2.0*LHV[index_H2])*/);
        ...
        Af = surface_area(rb);
        ae = Af*(xif[layer_1]*k_p(layer_1, Tf) + xif[layer_2]*k_p(layer_2, Tf) + xif[layer_3]*k_p(layer_3, Tf) + xif[layer_4]*k_p(layer_4, Tf))/(0.5*(xc[0] - xc[1]));
        ...
        Af = surface_area(xc[0] + 0.5*(xc[0] - xc[1]));
        ap = ap0 + ae - Af*(-2.0*(xi[layer_1][0]*k_p(layer_1, T->ve[0]) + xi[layer_2][0]*k_p(layer_2, T->ve[0])+ xi[layer_3][0]*k_p(layer_3, T->ve[0]) + xi[layer_4][0]*k_p(layer_4, T->ve[0]))/(xc[0] - xc[1])) - Sp[0];
        A->me[0][0] = -ap;
        A->me[0][1] = ae;
        b->ve[0] = -ap0*T->ve[0] - Af*(T_outer_boundary*2.0*(xi[layer_1][0]*k_p(layer_1, T->ve[0]) + xi[layer_2][0]*k_p(layer_2, T->ve[0])+ xi[layer_3][0]*k_p(layer_3, T->ve[0]) + xi[layer_4][0]*k_p(layer_4, T->ve[0]))/(xc[0] - xc[1])) - Su[0];
        /* --------------------------------------------------------------------- */
        ...
        for (i = 2; i <= (FINE_PARTICLE_MESH-1); i++)
        {
            ...
            aw = Af*(xif[layer_1]*k_p(layer_1, Tf) + xif[layer_2]*k_p(layer_2, Tf) + xif[layer_3]*k_p(layer_3, Tf) + xif[layer_4]*k_p(layer_4, Tf))/(0.5*(xc[j-2] - xc[j-1]));
            ...
            ae = Af*(xif[layer_1]*k_p(layer_1, Tf) + xif[layer_2]*k_p(layer_2, Tf) + xif[layer_3]*k_p(layer_3, Tf) + xif[layer_4]*k_p(layer_4, Tf))/(0.5*(xc[j-1] - xc[j]));
        ...
        aw = Af*(xif[layer_1]*k_p(layer_1, Tf) + xif[layer_2]*k_p(layer_2, Tf) + xif[layer_3]*k_p(layer_3, Tf) + xif[layer_4]*k_p(layer_4, Tf))/(0.5*(xc[FINE_PARTICLE_MESH-2] - xc[FINE_PARTICLE_MESH-1]));
        ...
        ap = ap0 + aw - Sp[FINE_PARTICLE_MESH-1];
